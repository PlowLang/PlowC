fun makefileText(outName: String, description: String = ""): String {
    return """
# ----------------------------
# Makefile Options
# ----------------------------
...
LLSOURCES ?= $(sort $(call rwildcard,$(SRCDIR),*.ll) $(EXTRA_LLSOURCES))
...
LINK_LLSOURCES ?= $(call UPDIR_ADD,$(LLSOURCES:%.ll=$(OBJDIR)/%.ll.src))

# files created to be used for linking
LDFILES ?= $(LDCRT0) $(LINK_CSOURCES) $(LINK_CPPSOURCES) $(LINK_ASMSOURCES) $(LINK_LLSOURCES)
...
$(OBJDIR)/%.ll.src: $$(call UPDIR_RM,$$*).ll $(USERHEADERS) $(MAKEFILE_LIST) $(DEPS)
	$(Q)$(call MKDIR,$(@D))
	$(Q)echo [compiling] $(call NATIVEPATH,$<)
	$(Q)$(CC) -S $(EZLLFLAGS) $(call QUOTE_ARG,$(addprefix $(CURDIR)/,$<)) -o $(call QUOTE_ARG,$(addprefix $(CURDIR)/,$@))
